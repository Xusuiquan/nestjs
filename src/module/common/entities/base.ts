...
export abstract class BaseEntity {
    ...
    @Column({type: 'varchar', name: 'create_by', length: 64, comment: '创建者'})
    public createBy: string;

    @ApiProperty({type: String, description: '创建时间'})
    @Column({type: 'varchar', name: 'create_time', length: 64, comment: '创建时间'})
    public createTime: string;

    @ApiProperty({type: String, description: '更新者'})
    @Column({type: 'varchar', name: 'update_by', length: 64, comment: '更新者'})
    public updateBy: string;

    @ApiProperty({type: String, description: '更新时间'})
    @Column({type: 'varchar', name: 'update_time', length: 64, comment: '更新时间'})
